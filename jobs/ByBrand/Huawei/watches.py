...
def getModelsName(write:bool = False):
    url  = "https://consumer.huawei.com/pk/wearables/"
    res = requests.get(url)
    soup = BeautifulSoup(res.text, 'html.parser')
    modelsDiv = soup.select('.series-item-card_wrap')
    data=[]
    for model in modelsDiv:
        a = model.select('a.btn--series-topdp[title="Learn More"]')
        data.append({
            'link' : model.select('a.btn--series-topdp[title="Learn More"]')[0]['href'],
            'name' : model.select('a.btn--series-topdp')[0].text.strip()
            })
    if write:
        with open('data/wearableNameLink.csv', 'w',encoding="utf-8",newline="") as file:
...
def getDetails():
    models = []
    with open('data/wearableNameLink.csv', 'r', encoding="utf-8") as file:
    ...
    for model in models:
        ...
        data.append(obj)
    try:
        with open('data/wearableDetails.csv', 'w',encoding="utf-8",newline="") as file:
            fieldnames = data[0].keys()
