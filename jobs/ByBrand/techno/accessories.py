...
def getModelsName(write:bool = False):
    modelsApi  = f"https://searchapi.samsung.com/v6/front/b2c/product/finder/global?type={typeCode}&siteCode=pk&start=0&num=500000&sort=recommended&onlyFilterInfoYN=N&keySummaryYN=N&specHighlightYN=N&familyId="
    ...
    # print(data)
    if write:
        with open('accessorieNameModel.csv', 'w',encoding="utf-8",newline="") as file:
...
def getDetails():
    models = []
    with open('accessorieNameModel.csv', 'r', encoding="utf-8") as file:
    ...
    print(data[0].keys()    )
    try:
        with open('accessorieDetails.csv', 'w',encoding="utf-8",newline="") as file:
            fieldnames = data[0].keys()
