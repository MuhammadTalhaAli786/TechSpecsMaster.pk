...
def getModelsName(write:bool = False):
    url  = "https://danytech.com.pk/collections/smart-watches"
    ...
    if write:
        with open('data/watchesNameLink.csv', 'w',encoding="utf-8",newline="") as file:
...
def getDetails():
    models = []
    with open('data/watchesNameLink.csv', 'r', encoding="utf-8") as file:
    ...
    for model in models:
        ...
        data.append(obj)
    try:
        with open('data/watchesDetails.csv', 'w',encoding="utf-8",newline="") as file:
            fieldnames = data[0].keys()
