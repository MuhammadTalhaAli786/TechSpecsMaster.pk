...
def getModelsName(write:bool = False):
    url  = "https://danytech.com.pk/collections/mobile-power-banks"
    ...
    if write:
        with open('data/powerBankNameLink.csv', 'w',encoding="utf-8",newline="") as file:
...
def getDetails():
    models = []
    with open('data/powerBankNameLink.csv', 'r', encoding="utf-8") as file:
    ...
    for model in models:
        ...
        data.append(obj)
    try:
        with open('data/powerBankDetails.csv', 'w',encoding="utf-8",newline="") as file:
            fieldnames = data[0].keys()
